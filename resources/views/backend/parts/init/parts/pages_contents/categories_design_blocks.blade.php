    ...
    @foreach($categories_current_design_blocks as $design_block)
        <li data-order="{{$design_block->order}}" data-id="{{$design_block->id}}">
            ...
            <div class="collapsible-body">
                @if(count($design_block->categories_pages_blocks_contents))
                    <form onkeypress="return event.keyCode != 13;">
                        @foreach($design_block->categories_pages_blocks_contents as $categories_pages_block_content)
                        ...
                        @endforeach
                        <button class="btn waves-effect waves-light green" type="button" name="action" onclick="updateCategoriesPagesDesignBlockContent(this)">Save
                ...
                @if($design_block->design_block->children)
                    <div class="row">
                        ...
                        <div class="col s2">
                            <button class="btn waves-effect waves-light" type="button" name="action" data-parent-id="{{$design_block->id}}" onclick="addCategoriesPagesChildrenDesignBlock(this)">Add
